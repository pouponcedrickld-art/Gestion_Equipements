    ...
    <div class="categories-page" ref="pageContainer">
      ...
      <div class="page-header animate-in">
        ...
        <div class="page-actions" v-if="hasRole(['super_admin', 'gestionnaire_stock_general'])">
          <Button label="Nouvelle Catégorie" icon="pi pi-plus" @click="showCreateDialog = true"
      ...
      <Dialog :header="editingCategorie ? 'Modifier la Catégorie' : 'Nouvelle Catégorie'"
        v-model:visible="showCreateDialog" :style="{ width: '500px' }" :modal="true" class="category-dialog">
        <form @submit.prevent="saveCategorie" class="category-form">
          ...
          <div class="form-actions">
            ...
            <Button type="submit" :label="editingCategorie ? 'Modifier' : 'Créer'" :loading="categorieStore.loading"
              :disabled="!canSubmit" />
          </div>
        </form>
      </Dialog>
        ...
        <div v-if="selectedCategorie" class="category-details">
            ...
            <div class="stats-grid">
              ...
              <div class="stat-item" v-if="selectedCategorie.statistiques.par_statut">
                <div class="status-breakdown">
                  <div v-for="(count, status) in selectedCategorie.statistiques.par_statut" :key="status"
                    class="status-stat">
          ...
          <div class="detail-section" v-if="selectedCategorie.equipements?.length > 0">
            ...
            <div class="equipements-list">
              <div v-for="equipement in selectedCategorie.equipements.slice(0, 5)" :key="equipement.id"
                ...
                <Badge :value="equipement.statut_global"
                  :class="`status-${equipement.statut_global.replace('_', '-')}`" />
              </div>
            </div>
          </div>
        </div>
...
const runInitialAnimations = () => {
  // Simple animation handling without GSAP for now
}
